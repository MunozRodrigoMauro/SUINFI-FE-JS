...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (alive) setStats((s) => ({ ...s, users: { total: 0, loading: false } }));
      }
      try {
        const plist = await getProfessionals();
        const arr = Array.isArray(plist) ? plist : plist?.items || [];
        if (alive) setStats((s) => ({ ...s, pros: { total: arr.length, loading: false } }));
      } catch {
        if (alive) setStats((s) => ({ ...s, pros: { total: 0, loading: false } }));
      }
      try {
        const { data } = await axiosUser.get(`${API}/services`);
        if (alive) setStats((s) => ({ ...s, services: { total: (data || []).length, loading: false } }));
      } catch {
        try {
          const plist = await getProfessionals();
          const arr = Array.isArray(plist) ? plist : plist?.items || [];
  ...
  const userName = useMemo(() => user?.name || user?.email?.split("@")[0] || "Admin", [user?.name, user?.email]);
    ...
    <section className="min-h-screen bg-white text-[#0a0e17] pt-24 pb-20 px-4">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-3">
          <Stat label="Usuarios" value={stats.users.loading ? "…" : stats.users.total} hint="Registrados en la plataforma" />
          <Stat label="Profesionales" value={stats.pros.loading ? "…" : stats.pros.total} hint="Perfiles publicados" />
          <Stat label="Servicios" value={stats.services.loading ? "…" : stats.services.total} hint="Activos en el catálogo" />
        </div>

        <div className="mb-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <button onClick={openUsersDrawer} className="cursor-pointer rounded-2xl border bg-white p-5 text-left shadow-sm transition hover:shadow">
            <div className="text-xl font-semibold">👥 Usuarios</div>
            <p className="mt-1 text-sm text-gray-600">Gestionar usuarios registrados</p>
            <div className="mt-3 text-xs text-gray-400">Tip: buscá con <Kbd>nombre</Kbd>, <Kbd>email</Kbd> o <Kbd>rol</Kbd></div>
          </button>

          <button onClick={openProsDrawer} className="cursor-pointer rounded-2xl border bg-white p-5 text-left shadow-sm transition hover:shadow">
          ...
          <button onClick={openServicesDrawer} className="cursor-pointer rounded-2xl border bg-white p-5 text-left shadow-sm transition hover:shadow">
          ...
          {/* 🆕 Liquidaciones / Pagos */}
          <button onClick={() => navigate("/dashboard/admin/settlements")} className="cursor-pointer rounded-2xl border bg-white p-5 text-left shadow-sm transition hover:shadow">
            <div className="text-xl font-semibold">💸 Pagos & Liquidaciones</div>
            <p className="mt-1 text-sm text-gray-600">Señas acreditadas, reembolsos y giros</p>
            <div className="mt-3 text-xs text-gray-400">Acceso al panel de liquidaciones</div>
          </button>
        ...
        <div className="mt-10 text-center text-sm text-gray-500">Último acceso: {new Date(user?.lastLogin || Date.now()).toLocaleString()}</div>
      </div>

      <Drawer open={openUsers} onClose={() => setOpenUsers(false)} title="Usuarios">
        <div className="mb-3"><SearchBox value={usersSearch} onChange={setUsersSearch} placeholder="Buscar por nombre, email o rol…" /></div>
        <Table
          cols={[
            ...
            { key: "createdAt", label: "Alta", render: (u) => (u.createdAt ? new Date(u.createdAt).toLocaleDateString() : <span className="text-gray-400">—</span>) },
          ]}
          rows={usersFiltered}
          getKey={(u) => u._id}
        />
      ...
      <Drawer open={openPros} onClose={() => setOpenPros(false)} title="Profesionales">
        <div className="mb-3"><SearchBox value={prosSearch} onChange={setProsSearch} placeholder="Buscar por nombre, email o categoría…" /></div>
        <Table
          cols={[
            { key: "user", label: "Profesional", render: (p) => (
              <div>
                <div className="font-medium">{p?.user?.name || "—"}</div>
                <div className="text-xs text-gray-500">{p?.user?.email}</div>
              </div>
            ) },
            { key: "categories", label: "Categorías", render: (p) => (p?.categories || []).length ? (
              <div className="flex flex-wrap gap-1">{p.categories.map((c, i) => (<span key={i} className="rounded-full border px-2 py-0.5 text-xs">{c}</span>))}</div>
            ) : (<span className="text-gray-400">—</span>) },
            { key: "services", label: "Servicios", render: (p) => <span>{(p?.services || []).length || 0}</span> },
          ]}
          rows={prosFiltered}
      ...
      <Drawer open={openServices} onClose={() => setOpenServices(false)} title="Servicios">
        <div className="mb-3"><SearchBox value={servicesSearch} onChange={setServicesSearch} placeholder="Buscar por nombre o categoría…" /></div>
        <Table
          cols={[
            ...
            { key: "price", label: "Precio", render: (s) => (s.price != null ? <span>{new Intl.NumberFormat("es-AR", { style: "currency", currency: s.currency || "ARS" }).format(s.price)}</span> : <span className="text-gray-400">—</span>) },
            { key: "durationMin", label: "Duración", render: (s) => (s.durationMin ? `${s.durationMin} min` : <span className="text-gray-400">—</span>) },
            { key: "owner", label: "Profesional", render: (s) => (
              <div>
